...
describe('AddGroupFormDialogAdapter.vue', () => {
  describe('mounting', () => {
    it('should set the form data to match the props data when it is passed', () => {
      const propsData = {
        initData: {
          name: 'something'
        }
      };
      const wrapper = shallow(AddGroupFormDialogAdapter, { propsData });
      expect(wrapper.vm.formData).to.eql(propsData.initData);
    });
  });
